...
Ocean.prototype.initSprites = function () {
    ...
    for (var p = 0; p < particleCount; p++) {
      ...
      var randomX = Math.random(),
          ...
          particle = new THREE.Vector4(pX, pY, pZ, pY)
...
Ocean.prototype.createOceanFarPlane = function () {
    this.oceanFarPlane = new THREE.Mesh(
        ...
        new THREE.ShaderMaterial({
            ...
            varying vec2 vUv;
            void main() {
                float stretch = min(1.0, (1.0 - pow(vUv.y, 2.0)) * 1.4);
                float stretch2 = min(0.7, (1.0 - pow(vUv.y, 2.0)) * 1.4);
...
Ocean.prototype.permuteStars = function () {
    var speed = 1000 / this.config.fps / 16666.666666666664;

    var vertex;
    zsprites += speed; //0.001;
    var zz = Math.abs(Math.floor(zsprites * 1000) % (waveDatas.length * 2) - waveDatas.length);
    var maxVert = this.sprites.geometry.vertices.length - (this.config.worldWidth / 4);
    inc = 0;

    for (var i = 0; i < maxVert; i++) {
        vertex = this.sprites.geometry.vertices[i];
        var large = Math.abs(noise(vertex.x * large_freq, vertex.y * large_freq, zsprites)) * 13;
        vertex.y = vertex.w + large;
    }

    this.sprites.geometry.verticesNeedUpdate = true;
    this.sprites.geometry.normalsNeedUpdate = true;

    this.sprites.geometry.computeFaceNormals();
}
...
Ocean.prototype.renderFrame = function () {
    ...
    if (should_render) {
        ...
        this.sprites.material.opacity = Math.min(0.7, Math.abs(Math.min(this.camera.position.y + 2.5, 0)) / 20);
        this.renderer.clear();
